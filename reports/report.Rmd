...
I established that most of the continuous data in the complete dataset was regarding video views. This dataset is a count of view by various metrics against each video in the step. However, given that I hadn't particularly explored it I was keen to use a some data I had viewed previously. In the student info dataset I had enrolled and un enrolled dates and use this to feature engineer a total enrolled time metric per student, expressed as enrolled date - un enrolled date, in days. I then attempted to correlate the enrolled time against the number of steps completed. This left me with 4374 students to work with and 385558 completed steps, a ratio of 1 to 88.147, which is larger than the unique steps in the course, at 58. The workings are shown in the next figure.
...
To investigate I created some plots showing total steps by learner which was a count of steps completed against learner id, were giving me count of up to approximately 1500, so i chose to query the data more directly as per below.
...
The original data had been provided in 7 data files where the number had been provided in the file name. Given that this was more than the 5 times of some data for some students i hadn't expected see i decided to investigate whether i had multiple enrollments across files. For want of a better phrase I had added that value into the source data table as "stage_id", and then has used it no further. 
...
2. Do students with previous higher attainment complete more steps of the course successfully? Or does the data merely prove there is more of them on the course?
